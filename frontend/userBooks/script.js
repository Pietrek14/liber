...
const init = async () => {
	...
	data.borrows.forEach( async (borrow) => {
		...
		tile.setAttribute("class", "d-flex flex-column")
		...
		p1.innerText = `${borrow_date.toISOString().substring(0, 10)}`;
		p2.innerText = `${receive_date.toISOString().substring(0, 10)}`;
		p3.innerText = `${giveback_date.toISOString().substring(0, 10)}`;
